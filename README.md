...
1. Open a terminal window and navigate to `final/subgraph-locations`.
...
1. Open a second terminal window, and navigate to `final/subgraph-reviews`.
...
1. Open a third terminal window, and navigate to `final/router`.
1. Run `rover dev --supergraph-config supergraph.yaml --router-config router.yaml`. This will start the router at `http://localhost:4000/`.
1. In a web browser, open Sandbox at `http://localhost:4000`. You should be able to run queries against your router. Some test queries are included in the following section.
